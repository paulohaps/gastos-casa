...
if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        navigator.serviceWorker.register('./sw.js').catch(() => console.log('Sem PWA por enquanto.'));
    });
}
...
function inicializarApp() {
    ...
    window.GastosDashboard?.init({
        ...
        getCurrentCalendarMonth: () => mesAtualVigente,
...
async function carregarMesesDisponiveis(mesFoco = null) {
    try {
        ...
        showToast(error?.message || 'Erro de conexão', true);
    }
}

function mudarMes() {
    carregarDados(document.getElementById('seletorMes').value);
}
...
async function carregarDados(mesParam) {
    ...
    try {
        ...
        window.GastosDashboard?.renderOverview(dadosMesAtual);
...
function dadosAnterioresComparaveis() {
    return window.GastosDashboard?.getComparablePrevious() || [];
}

function textoPeriodoComparativo() {
    return window.GastosDashboard?.getComparisonLabel() || 'mês anterior';
}

function renderInsights() {
    return window.GastosDashboard?.renderInsights();
}

function gerarResumo() {
    return window.GastosDashboard?.copySummary();
}
